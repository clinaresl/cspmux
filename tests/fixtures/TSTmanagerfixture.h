...
class ManagerFixture : public ::testing::Test {
    ...
    protected:
        ...
        void randVarVals (int n, vector<string>& name, vector<vector<value_t<int>>>& values) {
        ...
        vector<pair<int,int>> cross_product (int idx1, int idx2,
            ...
            for (auto val1 : values[idx1]) {
                for (auto val2 : values[idx2]) {
                    output.push_back (pair<int, int> {val1.get_value (),
                        val2.get_value ()});
                }
            }
        ...
        pair<int, int> get_bounds (size_t i, vector<vector<value_t<int>>>& values) {
